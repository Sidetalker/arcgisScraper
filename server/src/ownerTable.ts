import { Feature, FieldDefinition as ArcgisFieldDefinition } from './arcgisClient';
...
type ImportantColumn = (typeof IMPORTANT_COLUMNS)[number];
type SupplementalColumn = (typeof SUPPLEMENTAL_COLUMNS)[number];

export type OwnerColumn = ImportantColumn | SupplementalColumn;

export type OwnerRowAttributes = Record<OwnerColumn | '__rowId' | 'PropertyScheduleText', string>;

const OWNER_FIELDS: OwnerColumn[] = [...IMPORTANT_COLUMNS, ...SUPPLEMENTAL_COLUMNS];
...
function extractOwnerNames(attrs: Record<string, unknown>): string[] {
...
function normalizeComplexName(attrs: Record<string, unknown>): string {
...
function extractUnit(attrs: Record<string, unknown>): string {
...
function splitOwnerName(rawName: string): OwnerNameParts {
...
function aggregateOwnerName(parts: OwnerNameParts): string {
...
export function buildOwnerFields(): ArcgisFieldDefinition[] {
...
export function formatOwnerTable(
  features: Feature<Record<string, unknown>>[]
): Feature<OwnerRowAttributes>[] {
  const rows: Feature<OwnerRowAttributes>[] = [];
  ...
  features.forEach((feature, index) => {
    ...
    const physicalAddress =
      ensureString(attrs.SitusAddress) || ensureString(attrs.BriefPropertyDescription);
    ...
    const {
      ...
      state,
      postcode,
    } = parseOwnerAddress(attrs.OwnerContactPublicMailingAddr);
    ...
    names.forEach((rawName, nameIndex) => {
      ...
      const mailingLines = [addressLine1, addressLine2, cityLine].filter((line) =>
        Boolean(cleanWhitespace(line))
      );
      ...
      const rowAttributes: OwnerRowAttributes = {
